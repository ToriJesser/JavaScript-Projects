...
function hideGameMsg() {
    clearMsg() // clear the text from the message consol
    document.getElementById('gameMsgBox').style.display = 'none'; // hide the div
}

// this function will write text to the game message controls
...
function determineAvatar() {
    ...
    if (active == "Player 1") { // check which player is active and their correpsonding avatar
...
function avatarPlaced() {
    ...
    if (activePlayer == "Player 1") { // once active player selects a swuare change the active player
...
// this function will get lusted of previous moves
...
// this function will make the winning squares light up in celebrations
...
function squareSound() {
    ...
    sound.play();
    setTimeout(function() {sound.pause();}, 400); //add delay to thses to keep sounds short
...
function blink() {
    ...
    setTimeout(function() {body.style.backgroundColor = '#ffffff';}, 1010);
}
